...
Source builds default to SDK 1.3.0; the gate wire protocol remains 1.1. `app-a` opts into [application telemetry](../docs/application-telemetry.md) with SDK 1.3, while `app-b` retains the original uninstrumented lifecycle API. Explicit older SDK builds remain supported. The opt-in C++ QEMU runner lives separately in [qemu-runners/cpp-runner/](../qemu-runners/cpp-runner/README.md), with its explicit device/network boundary described in [M7 backends](../docs/m7-backends.md).
