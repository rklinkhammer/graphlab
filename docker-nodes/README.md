...
The [M6 completion bundle](../docs/validation/m6/completion.md) adds complete local SDK releases 1.0.0 and 1.1.0 and all four independent app/version images. Select the exact installed SDK with `-DLAB_SUPPORT_VERSION=1.0.0` or `1.1.0` for those frozen releases. Gate minors 0 and 1 interoperate in both directions under the frozen and current controllers. The current controller validates the actual wire major before releasing any node. Preserve `qualification/artifacts/linux-arm64-m6-completion/` alongside the original bundles; it contains the pinned SDK/image bytes, not just version names.

Source builds default to SDK 1.6.0; the gate wire protocol remains 1.1. `app-a` opts into [application telemetry](../docs/application-telemetry.md) with SDK 1.3, while `app-b` retains the original uninstrumented lifecycle API. Explicit older SDK builds remain supported. The opt-in C++ QEMU runner lives separately in [qemu-runners/cpp-runner/](../qemu-runners/cpp-runner/README.md), with its explicit device/network boundary described in [M7 backends](../docs/m7-backends.md).

For a complete current example, follow [Run the console for a specific topology](../docs/run-console.md), which builds the opt-in `app-message-target` and `app-messages` fixtures and launches their capture-first Docker triangle.
