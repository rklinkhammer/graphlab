...
test('message fixture endpoint reports correlate to verified Linux PCAPNG identities',async({page,request})=>{
 ...
 const root='/var/tmp/gl6-messages-20260924',base='http://127.0.0.1:18099',config=`${process.env.HOME}/.lima/graphlab/ssh.config`;
 ...
 const m=exact.matches[0],proof=JSON.parse(ssh(`sudo python3 /tmp/graphlab-messages-20260924/tests/qualification/message_capture.py ${root}/state/captures ${runId} '${JSON.stringify(event)}' ${m.artifactId} ${m.blockOffset} ${m.packetIndex}`));expect(proof.wireIdentifierVerified).toBe(true);
 ...
 ssh('sudo systemctl stop graphlab-messages-agent');ssh(`sudo systemd-run --unit=graphlab-messages-agent --property=KillMode=process /tmp/graphlab-messages-20260924/build/dev/lab-agent --socket ${root}/rpc/agent.sock --topologies ${root} --lock ${root}/artifacts.lock.json --state ${root}/state --allow-uid 501`);
