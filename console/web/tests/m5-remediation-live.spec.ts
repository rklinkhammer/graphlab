...
test('M5 skewed browser clock and independent refresh recovery',async({page})=>{
 test.skip(!process.env.GRAPHLAB_M5_LIVE,'Requires dedicated Linux M5 services');test.setTimeout(120000);
 await page.addInitScript(()=>{const now=Date.now.bind(Date);Date.now=()=>now()-60000;});
 ...
 await panel.getByLabel('Duration seconds').fill('5');await panel.getByRole('button',{name:'Preview fault placement'}).click();await expect(panel.getByRole('button',{name:'Apply directional fault'})).toBeVisible();await panel.getByRole('button',{name:'Apply directional fault'}).click();
 await expect(panel.getByRole('status')).toContainText('succeeded',{timeout:15000});await page.route('**/telemetry/query',route=>route.fulfill({status:503,contentType:'application/json',body:JSON.stringify({error:'injected_history_outage'})}));
 await expect(panel.getByRole('alert').filter({hasText:'history:'})).toBeVisible();
 await expect(panel.getByText(/a-s a-to-b.*removed/)).toBeVisible({timeout:15000});
 ...
 await page.unroute('**/telemetry/query');
 await expect(panel.getByRole('alert').filter({hasText:'history:'})).toHaveCount(0,{timeout:10000});
 await expect(panel.getByRole('table')).not.toContainText('stale',{timeout:10000});
 await page.screenshot({path:resolve(root,'docs/validation/m5-remediation-console.png'),fullPage:true});
