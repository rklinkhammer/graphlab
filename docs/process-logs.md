...
[Increment F qualification](validation/increment-f/README.md) freshly verifies a real stopped-workload restart creating a distinct log generation while earlier artifacts remain readable, plus native QEMU/runner byte comparisons and real ENOSPC rollback/retry. Actual Docker log-driver rotation remains outside that qualification.
