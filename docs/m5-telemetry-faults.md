...
The correlated timeline supports filtering, older-record pages of 100 items, and return-to-newest. Paging freezes the returned event snapshot so background polling cannot move the page. Pagination is within the backend's bounded retained event response. Separate [application telemetry](application-telemetry.md) and [finalized packet history](packet-history.md) now supply their own measurement contracts and bounded stores; they do not derive application latency, message identity or packet delivery from interface counters.
