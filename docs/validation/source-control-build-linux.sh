...
app_source=/tmp/graphlab-source-20260923
...
"$app_cmake" --build "$app_source/build/dev" --target lab-agent lab-api lab-terminal lab-capture m2_tests source_control_tests application_telemetry_tests lab_lifecycle -j 4
"$app_source/build/dev/source_control_tests" "$app_source"
"$app_cmake" --install "$app_source/build/dev/packages/lab-support" --prefix "$app_sdk"
for node in app-a app-sources; do
"$app_cmake" -S "$app_source/docker-nodes/$node" -B "$app_source/build/$node" -G Ninja -DCMAKE_MAKE_PROGRAM=/var/tmp/graphlab-m6-tools/root/usr/bin/ninja -DCMAKE_PREFIX_PATH="$app_sdk" -DLAB_SUPPORT_VERSION=1.5.0 -DOPENSSL_INCLUDE_DIR=/var/tmp/graphlab-m6-tools/root/usr/include -DOPENSSL_CRYPTO_LIBRARY=/usr/lib/aarch64-linux-gnu/libcrypto.so.3
...
sudo docker build --build-arg BASE_IMAGE=sha256:be5f4184e81e402c997478da98ee34e208b84922da122a94948f033b3f95a458 -t "graphlab-source/$node:qualification" "$app_source/build/image-$node"
done
